...
void mqtt_client::publish(uint8_t max_qos, bool retained, std::shared_ptr<mqtt_publish> packet_ptr) {
	...
	g_server_p->write_channel(_channel_id, publish_ptr);
}
...
void mqtt_client::handle_pubrec(std::shared_ptr<mqtt_pubrec> packet_ptr) {
	// I'm senter
	...
	g_server_p->write_channel(_channel_id, pubrel_ptr);
}

void mqtt_client::handle_pubrel(std::shared_ptr<mqtt_pubrel> packet_ptr) {
	// I'm receiver
	...
	g_server_p->write_channel(_channel_id, pubcomp_ptr);
}
